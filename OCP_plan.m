function [pos1sol,pos2sol,t] = OCP_plan(Lsize,Hsize,d,P,D,L,speed,T,rmax,pos0,yaw0,N)
...
gamma_distsq_penalty = @(x) gamma_size*(pi/2-atan(rise_fac_penalty*((x/rad_g^2)-1)))/pi;
...
penalty_disc = zeros(length(yrange),length(xrange));
...
for i=1:length(yrange)
    for j=1:length(xrange)
        for k=1:(N+1)
            if penalty_disc(i,j)
                distsq=(pos1(k)-Cellpos1(i,j))^2+(pos2(k)-Cellpos2(i,j))^2;
                penalty = penalty + gamma_distsq_penalty(distsq);
            end
        end
    end
end

opti.minimize(cost+penalty);
