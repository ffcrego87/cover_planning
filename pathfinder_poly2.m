function waypoint_coordinates = pathfinder_poly2(start_point, end_point, P)
...
initial_combined_nodes = zeros(size(P,1)+2,3);

%Create initial unvisited nodes array
initial_combined_nodes(1,1:2) = [start_point(1), start_point(2)];
initial_combined_nodes(2:size(P,1)+1,1:2) = P;
initial_combined_nodes(size(P,1)+2,1:2) = [end_point(1), end_point(2)];
...
for reference_node_ID = 1:size(initial_combined_nodes,1)
    ...
    for target_ID = 1:size(initial_combined_nodes,1)
        ...
        visibility = line_of_sight(observer_state, current_target_node, P);
