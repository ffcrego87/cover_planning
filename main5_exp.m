...
ALOP_mat     = zeros(nSamples, 3);
UAR_mat      = zeros(nSamples, 3);
compTime_mat = zeros(nSamples, 3);
surf_mat     = zeros(nSamples, 3);  % New: Surface coverage metric
...
for sample = N
    ...
    % --- MST - Square ---
    tic;
    [pos1sol_mst, pos2sol_mst, t_mst] = Boustro_backtrack_plan(Lsize, Hsize, d, P, D, L, speed);
    comp_time_MST = toc;
    ...
    ALOP_mat(sample,:)     = [ALOP_tsp,   ALOP_hex,   ALOP_mst];
    UAR_mat(sample,:)      = [UAR_tsp,    UAR_hex,    UAR_mst];
    compTime_mat(sample,:) = [comp_time_TSP, comp_time_MST_hex, comp_time_MST];
    surf_mat(sample,:)     = [Surf_tsp, Surf_hex, Surf_mst];  % New metric
end
...
algorithms = {'TSP', 'MST-Hex', 'MST-Square'};
